...
- **Powershell QR Code Generation**: Images are displayed entirely in the CLI. most suitable on windows computers with policies that severly restricts browsers and software installtions.
...
## Getting Started (Powershell Generator Only)

### Prerequisites

- Needs QRCoder.dll for .NET 4.0 in the same folder or path available here https://www.nuget.org/packages/QRCoder
- A windows machine or enviorment that can run powershell scripts

## Usage

### Data Sender Mode
Here is an example call to the script and the defaults. FilePath is the only mandatory param.

Variables:
    -FilePath "the file you want to send"
    -VerticalScale "The ratio the QR is scales vetically as a multiple"
    -HorizontalScale "The ratio the QR is scales horizontally as a multiple"
    -DarkChar "The char you want to represent dark parts of the QR code. ' ' or '.' work best for the default powershell colours"
    -LightChar "The char you want to represent light parts of the QR code. '#' or '%' work best for the default powershell colours"
	-ChunkSize "data size in KBs you want to send per QR, note that this is a really lossy format so you will want to keep it low"
	-ChunkDelayMs "Delay each finished chunk is kept up on the screen for, sometimes needs to be increased when generation is slow"
